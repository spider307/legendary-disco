...
options = optimoptions('ga', ...
    'PopulationSize', 50, ...     % 种群规模50
    'MaxGenerations', 200, ...     % 最大迭代进化10代
    'EliteCount', 10, ...         % 精英个体保留10个
...
toc;
fprintf('第二问的有效遮挡时间为 %.3f秒\n',-fval)  % 显示最优适应度值
fprintf('决策变量:\n无人机极角 %.3f°\n无人机飞行速率 %.3fm\\s\n投放时间 %.3fs\n延迟时长 %.3fs',x_1(1)*180/pi,x_1(2),x_1(3),x_1(4))
...
function TIME=shi_ying_du(x,A)
    ...
    s3=3;                          % 云团下沉速度
    g=9.8;                         % 重力加速度
    t_in=t_drop+dt_det;            % 云团开始运动时间
    t_out=t_in+20;                 % 云团结束运动时间（总时长20s）
    ...
    % 计算云团初始位置（考虑飞机运动 + 下落）
    cloud_init_pos = FY1_init_pos+t_in*FY1_vel-[0,0,0.5*g*dt_det^2];

    % ------------ 4. 生成时间轴与轨迹 ------------
    dt_points = 150;                     % 时间采样200个点
    t_span = linspace(t_in, t_out, dt_points)';  % 时间向量 t_in ~ t_out
    
    % 云团运动轨迹（下沉）
    cloud_traj = cloud_init_pos - s3 * [zeros(dt_points,2), (t_span-t_in)];
    % 导弹运动轨迹（匀速飞向原点）
    miss_traj = miss_int_pos - miss_spd * t_span * miss_int_pos / norm_miss_origin;
    ...
    for i = 1:size(A,1)
        ...
        % 距离 < 10 表示被遮挡
        d2 = dist_point_segment_vec(cloud_traj, A_mat, miss_traj) - 10;
        
        diff_d2 = diff(sign(d2));  % 计算符号变化（判断是否穿越距离=10）
        ...
        elseif isscalar(sign_change_idx)
            ...
            x1 = t_span(idx); y1 = d2(idx);
            x2 = t_span(idx+1); y2 = d2(idx+1);
            ...
            % 求进入遮挡时刻
            x1 = t_span(idx1); y1 = d2(idx1);
            x2 = t_span(idx1+1); y2 = d2(idx1+1);
            root_left = x1 - y1*(x2-x1)/(y2-y1);

            % 求离开遮挡时刻
            x1 = t_span(idx2); y1 = d2(idx2);
            x2 = t_span(idx2+1); y2 = d2(idx2+1);
    ...
    % ------------ 6. 输出适应度值 ------------
    TIME=min(T);  % 取8个点中【最小遮挡时间】（保证所有点都能被遮挡）
...
function y=excel(x,A)
    ...
    for i = 1:size(A,1)
        ...
        elseif isscalar(sign_change_idx)
            ...
            if diff_d2(idx) == 2
                % 符号：- → +  离开遮挡
                T(i,:) = [root - t_in,root,t_in];
            else
                % 符号：+ → -  进入遮挡
                T(i,:) = [t_out - root,t_out ,root];
            end
    ...
    % ------------ 6. 输出适应度值 ------------
    [TIME,idx]=min(T(:,1));  % 取8个点中【最小遮挡时间】（保证所有点都能被遮挡）
    y = T(idx,:);
end